  ...
  function wrapWithTooltip(content, project, index) {
    ...
    return (
      <Tooltip
        ...
        content={(<TooltipContent src={`/images/projects/min/${project.images[0]}`} className='object-cover' />)}
        placement={placement}
        classNames={{
                ...
                >
                  ...
                  <ProjectFooter className='flex flex-col h-full'>
                    ...
                    <div className='flex gap-2 mt-3 text-ellipsis overflow-hidden'>
                      {project.tags.map(tag => <Chip key={tag} size='sm' variant='bordered'>{tag}</Chip>)}
                    </div>
                  </ProjectFooter>
                </ProjectCard>
              </motion.div>
